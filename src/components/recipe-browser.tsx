...
import { RecipeSelectionProvider } from "@/components/recipe-selection";
import { SelectableCard } from "@/components/selectable-card";
import { BatchActionBar, SelectionToggle } from "@/components/batch-action-bar";
...
export async function RecipeBrowser({
  ...
  return (
    // The provider wraps the whole browser so the selection survives paging —
    // its position in the tree is stable across same-route navigations.
    <RecipeSelectionProvider>
      <div className="flex flex-col gap-4">
        ...
        {items.length === 0 ? (
          ...
          <>
            ...
            <div className="grid gap-4 [grid-template-columns:repeat(auto-fill,minmax(18rem,1fr))]">
              {items.map((recipe) => (
                // The card stays a server component; SelectableCard only wraps it.
                <SelectableCard
                  key={recipe.id}
                  recipeId={recipe.id}
                  title={recipe.title}
        ...
        <BatchActionBar pageRecipeIds={items.map((recipe) => recipe.id)} />
      </div>
    </RecipeSelectionProvider>
  );
}
